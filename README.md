...
- **In game (no Sodium):** Mods menu → Borderless Window → Config. Four options: Screen Mode, F11 Mode, FPS Overlay, and FPS Overlay Position. Changes apply instantly.
- **In game (with Sodium):** Video Settings → the "Screen Mode" option (replaces Sodium's fullscreen checkbox) and the "Borderless Window" section for the F11 Mode and FPS overlay options.
...
Un mod ligero client-side para **NeoForge 1.21.1** que agrega **pantalla completa sin bordes** a Minecraft, reemplazando el fullscreen exclusivo con un sistema de 3 modos — más un contador de FPS configurable.
...
- **Contador de FPS**: el modo Simple muestra los FPS actuales; el modo Extendido muestra una sola línea con los FPS actuales más el Mín / Promedio / Máx de los últimos 15 segundos. Posición configurable (cualquier esquina de la pantalla). Las estadísticas muestrean el contador de FPS del propio juego, así que siempre coinciden con F3 y respetan el VSync / límite de FPS. Se oculta solo mientras F3 está abierto.
...
- **Integración con Sodium** (opcional): con [Sodium](https://www.curseforge.com/minecraft/mc-mods/sodium) 0.8.x instalado, el checkbox de "Pantalla completa" de su menú de video se reemplaza por un selector de "Modo de pantalla", y la sección propia del mod gana las opciones de "Modo de F11" y del contador de FPS. Construido completamente sobre la Config API oficial de Sodium — sin mixins frágiles a sus clases internas. También funciona bien con Reese's Sodium Options.
...
- **En juego (sin Sodium):** menú de Mods → Borderless Window → Config. Cuatro opciones: Modo de pantalla, Modo de F11, Contador de FPS y Posición del contador. Los cambios aplican al instante.
- **En juego (con Sodium):** Ajustes de video → la opción "Modo de pantalla" (reemplaza el checkbox de fullscreen de Sodium) y la sección "Borderless Window" para el Modo de F11 y las opciones del contador de FPS.
